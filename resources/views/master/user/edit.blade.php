        ...
        <div class="flex items-center py-3">
            <ol class="flex items-center whitespace-nowrap">
                <li class="flex items-center text-base text-gray-800">
                    <a href="{{ route('user-index') }}" class="hover:text-blue-600 font-medium">User</a>
    ...
    <div class="max-w-4xl mx-auto mt-8 px-4 sm:px-6 lg:px-8">
        <div class="w-full bg-white rounded-3xl shadow-xl p-8 space-y-6">
            ...
            <div class="text-center">
                ...
                <h2 class="mt-3 text-2xl font-bold text-gray-800">Edit User</h2>
            ...
            <!-- Form -->
            <form action="{{ route('user-update', $user->user_id) }}" method="POST" class="space-y-4" aria-label="Edit user form">
                @csrf
                @method('PUT')
                        ...
                        <select name="province" id="province"
                            ...
                            @foreach ($provinces as $province)
                                <option value="{{ $province['nama'] }}"
                                    {{ old('province', $user->province) == $province['nama'] ? 'selected' : '' }}
                                    data-id="{{ $province['id'] }}">
                                    {{ $province['nama'] }}
                                </option>
                            @endforeach
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="mt-2 block w-full rounded-xl border-gray-300 shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent py-2 px-3 text-gray-800 resize-none">{{ old('address', $user->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                ...
                fetch(`https://ibnux.github.io/data-indonesia/kota/${provinceId}.json`)
                    ...
                    .then(data => {
                        ...
                        if ("{{ old('city', $user->city) }}") {
                            citySelect.value = "{{ old('city', $user->city) }}";
            ...
            citySelect.addEventListener('change', function() {
                ...
                fetch(`https://ibnux.github.io/data-indonesia/kecamatan/${cityId}.json`)
                    ...
                    .then(data => {
                        ...
                        if ("{{ old('kecamatan', $user->kecamatan) }}") {
                            districtSelect.value = "{{ old('kecamatan', $user->kecamatan) }}";
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
            ...
            if ("{{ old('province', $user->province) }}") {
                provinceSelect.value = "{{ old('province', $user->province) }}";
